...
def one_plus_one_ea_onemax2(n, c, max_iterations):
    #random initial solution
    x=[random.randint(0, 1) for _ in range(n)]
    iteration=0
    while iteration<max_iterations:
        iteration=iteration+1
        #each bit is flipped independently with probability c/n
        y=x.copy()
        for i in range(n):
...
def one_plus_one_ea_leadingones2(n, c, max_iterations):
    #random initial solution
    x=[random.randint(0, 1) for _ in range(n)]
    iteration=0
    while iteration<max_iterations:
